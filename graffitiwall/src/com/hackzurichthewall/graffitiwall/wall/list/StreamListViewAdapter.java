...
/**
 * Adapter connecting the main list for comments and pictures with the actual
 * data.
 * 
...
public class StreamListViewAdapter extends ArrayAdapter<AbstractContent> {
	...
	/**
	 * Defines the different supported types of content. NOTICE: "ABSTRACT" is
	 * just for {@link AbstractContent} which is abstract.
	 * 
	 * @author Johannes Glöckle
	 */
	public enum ContentType {
		TEXT_COMMENT, PICTURE_COMMENT, ABSTRACT
	}
	...
	/**
	 * Creates a new adapter with given list of items and layout resource to
	 * use.
	 * 
	 * @param context
	 *            current context
	...
	public AbstractContent getItem(int position) {
		...
		if (position < 0 || position > items.size()) { // checking argument
														// validity
			throw new IllegalArgumentException(
					"Position has to be between zero and number of items.");
		}
	...
	public int getItemViewType(int position) {
		...
		case ABSTRACT: // actually should never happen since "ABSTRACT" is only
						// for AbstractContent
			return -1;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) { // happens if there is no old item to recycle
			...
			case 0: // usual text comment
				convertView = mInflater.inflate(R.layout.list_item_comment,
						null);
				holder.setmCommentTxt((TextView) convertView
						.findViewById(R.id.tv_comment));
				// set font
				holder.mCommentTxt
					.setTypeface(FontFactory
							.getTypeface_NexaRustScriptL0(mContext));
				break;
			case 1: // picture comment
				convertView = mInflater.inflate(R.layout.list_item_picture,
						null);
				holder.setmPicture((ImageView) convertView
						.findViewById(R.id.iv_list_item_picture));
				break;
			}
		...
		getItem(position).fillViewHolder(holder); // fills the given view holder
													// with information
		return convertView;
	}
